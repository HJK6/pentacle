...
events and Claude/Codex working-state observations to hosta's existing
...
hysteresis only), supervised, and self-updating to the SHA hosta pins. See
`satellite.py`'s module docstring for the design.

## 0. Shared push secret (both ends)

`event.push` fails closed: hosta refuses every push unless a secret is configured
on BOTH ends and they match.

- **hosta** daemon env: `PENTACLE_EVENT_PUSH_SECRET=<secret>` (or kv
  `event_push.target_sha`'s sibling key `event_push.secret`). Set it in hosta's
...
hostc, `/Users/example/repos/pentacle-satellite` on hostb).

## 2a. hostc / WSL — systemd-user
...
`hostc:v2-*` / `hostb:v2-*` rows should appear in hosta's
...
quiet. hosta uses the pushed state as primary and resumes its existing central
...
end-to-end chat path with `request_stream_events` against prod hosta for a live
...
against prod hosta for a live Claude and Codex session on that host, and confirm
...
- `HOST` — fleet name; MUST match the registry host (`hostc`/`hostb`).
...
- `NO_AUTOUPDATE=1` — never git-checkout/exec-restart (stay on the running SHA;
  hosta still accepts pushes flagged-stale and alerts).
...
hosta-side kill switch: `--disable-event-push-ingest` makes `event.push` return
`ingest_disabled`; the satellite backs off and keeps its offsets (no loss).

## Serialized deploy-window pin step

Only in a Nexus-authorized deploy window, after the new hosta daemon has passed
...
with a readback before rolling hosta back to its prior accepted SHA:

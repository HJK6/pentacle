...
| `agent-orch report [--msg-id N] --status done [--result JSON \| --result-file FILE] [--reason R] [--report-id UUID] [--qa-verdict accept\|reject] [--target-sha FULL_SHA] [--qa-reviewed-scope TEXT --qa-gate-evidence-digest SHA256] [--qa-attestation-stream-id STREAM --qa-attestation-report-id REPORT] [--from-stream-id STREAM_ID] [--timeout N] [--terminate]` | Direct completion RPC for sub-agents. v2 accepts inline `ReportPayloadV1` through `--result` and persists it in `v2_reports`; `--qa-verdict` and `--target-sha` add typed top-level fields. Paired QA review flags require an explicit full `--target-sha` and compose `extras.qa_review` with candidate identity copied exactly from that SHA; partial or conflicting inputs fail locally. `--result-file` with QA flags is validated and submitted inline. With `--terminate`, closes the reporting session after `report.ok`. `--msg-id` is the inbox message this report answers; omitted defaults to `0`. A payload that fails validation prints the offending fields. |
...
`--cwd ABS_PATH` is not persisted on scheduled rows in this lane: the CLI rejects `--cwd` combined with either `--at` or `--delay` before calling `schedule_once` or `spawn_once`; use an immediate spawn for an explicit project directory, and omit `--cwd` to retain the target default.
...
An explicit `--self-close-on-completion` or `--no-self-close-on-completion` is serialized as the corresponding boolean for both immediate and scheduled handoffs. Handoff lineage satisfies the daemon's parent-lineage requirement, and the scheduled row re-emits the flag when it fires. An implicit leaderless spawn omits the lifecycle bit and defaults false; an explicit flag on a scheduled top-level spawn is refused before scheduling.
...
- **Requires lineage.** An immediate leaderless spawn omits either explicit self-close bit; a scheduled `--top-level` spawn with either flag is refused before scheduling. The daemon also rejects a leaderless payload carrying `self_close_on_completion=true` with `spawn.error error_code=self_close_unauthorized`. The flag is meaningless on top-level operator spawns — there is no leader to delegate from.
- **Not in role baselines.** Role baselines (`qa_baseline.md`, `documentation_baseline.md`, etc.) MUST NOT include this flag in their spawn boilerplate. Hidden seats with a real parent already default on; other visibility values default off. A lead who needs to keep a hidden worker open passes `--no-self-close-on-completion`. A test under `services/agent-orch/tests/test_role_baseline_safety.py` scans the live memory-repo baselines for any unconditional reference to the flag and fails if one appears.
...
- **Omitted flag follows visibility and lineage.** A hidden worker with a real parent gets `self_close_on_completion=True`; one with `--visibility default` or `--visibility nested` gets no self-close bit. Explicit `--no-self-close-on-completion` persists false for a parented worker or handoff. When a worker has no self-close permission and its leader is alive, `report --terminate` reaches `live_leader_refused` exit 6; the leader closes it after consuming the report.
...
`--qa-verdict`, `--target-sha`, or either QA review evidence flag, the CLI validates the file under the inline
...
For commissioned QA report construction, use `--qa-reviewed-scope` and
`--qa-gate-evidence-digest` with an explicit full `--target-sha` and
`--qa-verdict`; the canonical example and cycle semantics are in
[QA dispatch reject counter](../../docs/qa-dispatch-counter.md#adjudicate-and-diagnose).

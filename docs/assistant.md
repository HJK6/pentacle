...
- **Designate / replace / revoke:** only a connection-authenticated operator (the web or mobile client's operator credential), through the `assistant.lifecycle` verb. The web session context menu offers *Designate fleet lifecycle manager* and *Revoke fleet lifecycle manager*; the client reads back the target's current generation and the grant revision, then submits both.
- **Transfer:** only the current holder, with its own verified seat token and current generation: `agent-orch lifecycle transfer <stream> --reason <why> [--request-id <key>]`. Reusing the same request id and payload returns the original receipt. `agent-orch lifecycle inspect [--target <stream>]` shows an authenticated caller the grant, recent audit and a target's eligibility.

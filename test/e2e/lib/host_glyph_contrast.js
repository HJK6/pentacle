 ...
 try {
  ...
  for(const theme of ['dark','light']) {
   ...
   for(const state of ['normal','selected','hover']) for(const host of ['Thoth','Amaterasu','Merlin']) for(const type of ['avatar','filter']) {
    ...
    const observed=await session.eval(`(()=>{const row=document.querySelector('[data-proof-host="${host}"]');const el=row.querySelector('${type==='avatar'?'.s-machine-avatar':'button'}');const style=getComputedStyle(el);let bg=style.backgroundColor;let parent=el;while(bg==='rgba(0, 0, 0, 0)'&&(parent=parent.parentElement))bg=getComputedStyle(parent).backgroundColor;return {color:style.color,background:style.backgroundColor,effectiveBackground:bg,hover:el.matches(':hover'),width:el.querySelector('svg').getBoundingClientRect().width,label:el.title,strokes:[...el.querySelectorAll('[stroke]')].map(x=>getComputedStyle(x).stroke)};})()`);
    ...
    results.push({theme,state,host,type,...observed,contrast,pass:realLabels&&contrast>=3&&observed.color!=='rgb(255, 255, 255)'&&observed.background!==observed.color&&observed.width===15&&observed.label===host&&observed.strokes.some(c=>c===observed.color)&&observed.strokes.every(c=>c==='none'||c===observed.color)&&(state!=='hover'||observed.hover)});
   }
  }

...
function buildHtml(data) {
  // NOTE (Shared helper note): the transcript-event
  // and transcript-body mockup sections were removed when the legacy
  // renderEventContent / renderChatBody renderers were deleted from
  // chat_ui_state.js (the shared chat-core view is now the only
  // transcript renderer). The retained-helper sections below still render.
  const sections = [
  ...
  <div class="mock-page">
    <section class="mock-hero">
      <div>
        ...
        <p>Fixture-driven visual spec page generated from the current shared chat renderer. Every preview below is built from the same shared helpers used by the desktop UI, so these cards double as fixture previews and regression examples.</p>
      </div>
...
function renderDraftPreviewSection(cases) {
  return renderSection(
    ...
    'Remote draft-state previews rendered with shared markup.',
    cases.map((item) => renderCard({
      kind: 'draft',
      name: item.name,
      expected: item.expected_output,
...
function renderCombinedSection(data) {
  ...
  return renderSection(
    ...
    'Higher-level assembled screens using the same renderer pieces as shared.',

...
function registerTerminalIpc(ipcMain, config, client, { pty = null, execute = run, platform = process.platform, maxPtysPerConnection = null } = {}) {
  ...
  const { executionHost, executionCommand } = require('./execution_host');
  function target(host, args) {
    const resolved = executionHost(config, host);
    return executionCommand(resolved, [resolved.tmux, ...args], platform);
  }

...
// Each dashboard file (0dte.js, example-board.js, etc.) pushes its own entry.
...
function renderStalenessBadge(container, payload) {
  ...
  if (payload._transport_stale) {
    label = `⚠ Host unavailable — last known ${fmtAge(age)}`;
    color = '#d4a300'; // yellow
    title = 'optional dashboard transport is not connected. Showing local-cache data.';
    ...
    color = '#4ec275'; // green
    title = 'Source connected and publishing fresh data.';
  }
